...
class HVPOperatorFullSet(Operator):
    ...
    def __init__(self, model, grad_vec, device=torch.device('cpu'),
    ...
    def apply(self, vec):
        ...
        hvp_seq = torch.autograd.grad(self.grad_vec, self.model.parameters(), grad_outputs=vec_seq,
...
class HessianOperator(LinearOperator):
    ...
    def __init__(self, model, grad_vec, device=torch.device('cpu'),
        ...
        # TorchOp
        self.device = device
        self.hvp_op = HVPOperatorFullSet(model, grad_vec, device, weight_decay, dtype)
...
def eigenvalue_analysis(operator, k=10, tol=1e-6, max_iter=100, quiet=False):
    """Return largest EV in magnitude and smallest algebraic eigenvalue."""
    try:
        eigvals, eigvec = eigsh(operator, k, tol=tol, maxiter=max_iter)
